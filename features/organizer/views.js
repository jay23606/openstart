...
export function createOrganizerViews({ eventRegistrations, tierById }) {
  function event() {
    const body = `<form id="event-form" data-draft-key="organizer:new-event">
  ...
  function duplicate(source) {
    ...
    const body = `<p class="modal-note">Creates a private draft with registration options, questions, waiver, website content, sponsors, products, and shifted registration deadlines. Participants, payments, results, and staff are never copied.</p>
      <form id="duplicate-event-form" data-source-event-id="${source.id}" data-draft-key="organizer:duplicate:${source.id}">
        <p class="form-save-state" data-form-state data-state="saved" aria-live="polite">All changes saved</p>
  ...
  function roster(source) {
    ...
    return `<div class="dashboard-card roster-card">
      ...
      <div class="roster-toolbar"><input data-roster-search="${source.id}" type="search" aria-label="Search roster" placeholder="Search name, email, or bib"><select data-roster-status="${source.id}" aria-label="Filter roster by status"><option value="">All statuses</option><option>confirmed</option><option>pending</option><option>reserved</option><option>cancelled</option><option>expired</option></select><button class="subtle-button" data-add-participant="${source.id}" type="button">+ Manual entry</button><button class="subtle-button" data-export-roster="${source.id}" type="button">Export CSV</button><button class="subtle-button" data-race-day="${source.id}" type="button">Race day</button><button class="subtle-button" data-product-settings="${source.id}" type="button">Products</button><button class="subtle-button" data-pricing-settings="${source.id}" type="button">Pricing</button><button class="subtle-button" data-registration-settings="${source.id}" type="button">Form settings</button></div>
  ...
  return `
    <section class="dashboard">
      ...
      <div class="dashboard-card" data-dashboard-region="events">
      ...
      <div class="dashboard-card" data-dashboard-region="financials">
      ...
      <div class="dashboard-card" data-dashboard-region="communications">
