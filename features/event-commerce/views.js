...
export function createEventCommerceViews({ effectivePrice }) {
  function registrationSettings(event) {
    ...
    const body = `<form id="registration-settings-form" data-event-id="${event.id}" data-draft-key="event:${event.id}:registration-settings"><p class="form-save-state" data-form-state data-state="saved" aria-live="polite">All changes saved</p><label>Waiver text<textarea name="waiver_text" rows="6" placeholder="Leave blank to disable the waiver">${escapeHtml(event.waiver_text || "")}</textarea></label>
