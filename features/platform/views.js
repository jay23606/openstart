...
export function createPlatformViews() {
  ...
  function consolePage(data) {
    ...
    const telemetry = renderList((data.observability || []).slice(0, 30), (item) => `<article><div><b>${escapeHtml(item.message)}</b><small>${escapeHtml(item.source)} · ${escapeHtml(item.route)} · release ${escapeHtml(item.release)} · ${new Date(item.received_at).toLocaleString()}</small></div><span class="${item.severity === "error" || item.severity === "fatal" ? "health-bad" : "health-ok"}">${escapeHtml(item.severity)}</span></article>`) || '<div class="empty-state">No application errors recorded.</div>';
    const feedback = renderList((data.feedback || []).slice(0, 50), (item) => `<article><div><b>${escapeHtml(item.message)}</b><small>${escapeHtml(item.category)} · ${escapeHtml(item.route || "unknown view")} · ${new Date(item.created_at).toLocaleString()}</small></div><span>${escapeHtml(item.status)}</span></article>`) || '<div class="empty-state">No beta feedback submitted yet.</div>';
    ...
    return `<section class="platform-console">
      ...
      <div class="metric-grid platform-metrics"><div><span>Gross processed</span><strong>${money(metrics.grossCents)}</strong><small>${money(metrics.feeCents)} platform fees</small></div><div><span>Organizers</span><strong>${metrics.organizers}</strong><small>${metrics.activeEvents} active events</small></div><div><span>Reconciliation</span><strong class="${metrics.reconciliationAlerts + metrics.counterDrift ? "health-bad" : "health-ok"}">${metrics.reconciliationAlerts + metrics.counterDrift}</strong><small>${metrics.counterDrift} capacity drift · ${metrics.reconciliationAlerts} payment alerts</small></div><div><span>Operational failures</span><strong class="${metrics.failedDeliveries + metrics.failedProviderEvents + metrics.recentErrors ? "health-bad" : "health-ok"}">${metrics.failedDeliveries + metrics.failedProviderEvents + metrics.recentErrors}</strong><small>email + providers + application</small></div></div>
      ...
      <div class="dashboard-card"><div class="card-heading"><div><h2>Application errors</h2><p>Privacy-filtered browser and Edge Function signals grouped by fingerprint.</p></div></div><div class="operations-list compact">${telemetry}</div></div>
      <div class="dashboard-card"><div class="card-heading"><div><h2>Beta feedback</h2><p>Private, privacy-filtered reports submitted from the Help screen.</p></div></div><div class="operations-list compact">${feedback}</div></div>
